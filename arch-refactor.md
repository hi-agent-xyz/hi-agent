...
Four. The first two because the design as written could not be implemented as written; the
third because live testing showed the implemented choice was wrong; the fourth because the
mechanism the design named could not do the job the design gave it. Each is in `docs/arch/`
...
| **The host does not mark a hand-down as owed. Reaction reads it.** `LoopInput::Mail { owed }`, the `reply_owed` bool threaded through the loop, the second `render_batch` arm and `hand_down_to_cognition`'s `bool` return all go; mail renders as what was written. The rule — pass on what someone is waiting for, time what nobody asked for, and count the items because one message carries several — is a section of `reaction.md`. `agents.md`'s hand-down section is rewritten, and the sentence "the host is what knows the difference" is withdrawn | It could not know. The host knows *a* hand-down went out; whether *this* message answers it is a reading of message against request, and Reaction holds both — the request in its own long-lived session, the message in front of it. What the code actually did: set the flag on **every** turn a person spoke into (`render_human_from_batch` filters nothing, so "ok" hands down too), then give it to whichever Cognition message arrived first and spend it — so a background finding could be announced as the awaited answer while the answer behind it rendered bare. The three tests hand-wrote the boolean and asserted the two renderings; nothing tested the line that decided it. And at its best the flag bought **one sentence in a prompt** — no gate on `hi_say`, nothing enforced — so it read as a check while being guidance, which is the worst of both. Also closes gaps.md's layer 3 (batch-wide bool, not per item) by not having a bool. One mechanical fact stays in code: `mail_from_cognition`, which discharges a `back_in` promise made about work being under way — "the brain wrote" is a session-id comparison, not a reading |
...
There is no host scheduler. The host's timing surface is the loops that already exist —
**Cognition's glance-up** and the **reflection backoff** (the conversation had a third, its own
pulse; it was cut — see the table above) — and everything past
