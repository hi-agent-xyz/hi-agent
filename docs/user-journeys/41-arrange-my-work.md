...
- **自身维护并成一组**(2026-09-21 改的):代码不再自带「自身维护」组,Reflection 和没有
  subject 的会话画进排布里带 `upkeep: true` 的那一组。起因是本机上他那组「hi-agent 内部维护」
  和代码画的「自身维护」并排,同一类两个标题。只有单测;task-manager 会不会给他那组标上、
  首页是不是只剩一个标题,没亲眼看过。
