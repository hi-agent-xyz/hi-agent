...
2. **task-manager 每次跑都是一次整理**,不管它是被叫来干什么的(哪怕只是关一行):读
   ...
   `upkeep: true`,这些会话就画在那一组里。
3. 他自己有一组放 hi-agent 的事,就标那一组 —— 一类一个标题。没有,task-manager 自己留一组,
   名字用他的语言直白地说它是什么(中文就是「内部维护」这类),**不用我们内部的叫法**,
   组里不放他的任何任务;这一组没有成员也保留。
4. **他一句话都不用说。** 他能决定的只是这组叫什么、放在哪,说法和别的组一样。
5. `hi_set_home_groups` 的回执在没有任何一组带 `upkeep` 时另起一行明说 —— 和"哪些在办任务
   没进组"一样,是事后没人会提醒的那部分。
...
- **hi-agent 自己的维护活总有一组。** 不是代码画的,也不等他开口:task-manager 写的每份
  排布都标一个 `upkeep` 组。
...
- **Case E 的屏幕那一半**:记录里标上了(见文末复测),但首页是不是只剩一个标题、回顾和
  没有 subject 的会话是不是画进了「hi-agent 内部维护」,没亲眼看过 —— 无头 Chrome 打开
  `:12358/` 只拿到空舞台,`/render/view` 用的是样例数据,看不到这台实例。
- **总结技能的卡片标题**(`d95a0a10` 改的 title 规则):下一次 Reflection 交给 `skills-manager`
  总结一项技能时,卡片是不是写成「总结技能：…」,没等到。
...
## 实测 2026-09-22(本机 `data/`,dev 构建)· 自身维护那组没了

09-21 那次改动(`44384e30`)把代码自带的「自身维护」组删掉,规则当时写成"他有一组放
hi-agent 的事就标那一组,没有就让这些会话散在核心上"。起因是 09-18 他看到代码画的
「自身维护」和他那组「hi-agent 内部维护」并排,说「就一起就行」。

结果:标题直接没了,他问「"自身维护"这个分类呢?不见了?」。
...
他的判语:「是不再由代码管理,但是也不是用户管理啊,应该是 hi-agent 自己通过 agent 管理」。
改成 Case E:这一组 task-manager 总是留着,每次跑都是一次整理,回执在没有一组带 `upkeep`
时明说。`grouping.md` 那条误读不用人去改,因为这一组已经不靠依据文件里的哪句话了。

### 复测 2026-09-22(同一台,`9ca740d0` + `d95a0a10`,07:07Z 重启)

没有对它说任何话,只等下一个 task-manager。

- 07:15:40Z Cognition 起了 `task-manager-球追踪那行按-继续-收口`,brief 只是按他一句「继续」
  收口球追踪那一行,**和首页无关**。
- 07:22Z 它自己调了两次 `hi_set_home_groups`,回执里是 `hi-agent 内部维护 (5 + upkeep)`;
  第二次还顺手把刚开的 `vlx-seek-ball-mark-tracking-20260922` 放进了「视频理解」,剩一条
  `game-screenshot-parse-poc-20260920` 在组外。
- `home/groups.json` 里整棵树只有「hi-agent 内部维护」带 `upkeep: true`,其余组和成员没动。

也就是 Case E 的记录那一半成立:关一行的 manager 也做了整理,标到了他自己那组,没有另起
一组。屏幕那一半见上面「没看过的」。
