...
- ⚪ **这一问先在 `Pause` 里躺了 10 小时半——而这是对的。** 09:57:37 发出时 scene 卡在 `Pause`(前一天 38 分钟真实 402 留下的),**一个 turn 都没驱动**;10:02:45 重启后第一个 turn 就成功了。我一度把这判成缺陷,**判错了**:这台测试机的 token 在 LLM 服务端被手动加过预算,所以 gateway 算出的"没额度"与"实际还能用"必然不一致。生产环境两者一致,停下来等充值就是正确行为。整条撤回见 [gaps.md #21](gaps.md)。
