...
## 实测 2026-09-18 · Home 上那张图为什么糊

起因是一张截图:Home 缩到最大去看一张研究 view 的图,字全是糊的。

**原因不在渲染,在盘上的分辨率。**那张图是 `_shots/ref/jev-typesafe-20260918/jev-what-it-is.png`,
盘上 480×270。Home 的图片盒子是 240×135 CSS px,但 Home 缩放到 2×,屏幕又是 2× ——
算出来那张图被画在 **960 设备像素**宽的位置,对 480 的源做了整整 2 倍放大。**"缩进去看图"
恰好是唯一保证把图糊掉的动作**,而 `ZOOM_MAX = 2` 存在的理由写的就是"far enough in to read
a picture"。

**所以上面 9-08 那段里"盘上永远 480×270"今天改了**,改的只是密度,不是"帧是个常量"那条:
`TILE` 的 DPR 1.0 → 2.0(布局仍是 1280×720,截图回来 2560×1440,缩到缩略图是超采样而不是
对 1× 栅格化结果的重排),`THUMB_WIDTH` 480 → 960(= 最大的那个盒子在最大缩放、2× 屏上的
设备像素数)。`fills_the_tile` 同时管宽度,于是旧图跟当年的竖图走同一条自愈路。

**复测**(release 二进制,独立 `--data-dir`,端口 12360,把真库里那个 jev view 的目录拷进去):

- ✅ **盘上全是 `960x540`**:10 个 factory 具名 view 预热各一张,加 `POST /api/views/open`
  开出来的 jev 那张,11 张全部 960×540。
- ✅ **体积**:`factory/tasks` 8K → 32K,`factory/home` 24K → 124K,jev 96K → 247K。真库
  602 张的缓存按这个比例是 106 MB → 约 300 MB。
- ✅ **看得见的差别**:把旧的 480 那张按浏览器的做法双线性放大到 960,跟新图并排裁同一块 ——
  旧的标题带糊边、正文是一团灰;新的正文能读。
- ⚠️ **没在浏览器里看过 Home 画布**。对比是从盘上两张 PNG 推的,不是在 Home 上缩到 2× 看的。
