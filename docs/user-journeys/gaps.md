...
**仍然开着的,按疼痛排序**:🔴 **#3**(重启吃掉在途 worker 的回报,**本轮未复测**)· 🟡 #19(预览器平铺)· #12(turn 计数没有写者,却已被当作存活判据)· #25(Cognition/Reflection 不看 vendor gate)· #9 #23 #24(演出节奏与在场推断)· #6 #7 #11 #4残留。
...
**净判断:对话与记忆这一层明显变好了。**本轮唯一新出的 🔴(#20)当轮修掉,根是一句话:**`create_worker` 的契约与实现自相矛盾**——要求调用者先停下,交接才完成。**但清单上还剩一条 🔴:#3,而且它本轮一次都没测**;#20 修好之后在途 worker 变多,它的风险只增不减。**下一轮的第一件事应该是补测 #3。**
...
## 3 · 重启会吃掉在途 worker 的回报 🔴 · **本轮未复测;#20 修好后在途 worker 变多,风险更高**
