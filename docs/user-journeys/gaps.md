...
**这份文件收集"跑真机跑出来的"缺口**——不是设计分歧,不是代码没跟上设计(那是重构的工作本身,对着 [`docs/arch/`](../arch/) 改代码即可),而是**把 journey 当规格、对着真实运行的实例测出来的行为差距**。
...
## 14 · 空转的开销:Cognition 的 glance-up 没有退避,一夜 538 次 · ✅ **已解决 `a19789e`——不是加退避,是把周期臂删了**

> **2026-09-04 结论。** 这条曾判为"结构性缺陷仍在",修法一直被设想成"给 Cognition 加自适应退避,像 Reflection 那样"。实际的修法是删掉整个周期臂:再量一次全量帧日志,**1819 个纯定时器驱动的 turn 里 46% 一次工具调用和一条命令都没有**(有人真的问了的 turn 是 28%),而当初留着它的理由——重启后常驻职责丢失(本文 #1)——早就由 boot 那条臂修好了,两者在代码里一直是"共用一个旋钮的两个机制"。退避会让一个不该存在的东西变便宜;删掉它让成本归零。下方原始观测保留不动。
...
- **第 3 层后来换了个方向处理。** 当时的判断是"再加一道一次性的框同样会被剪掉",没动。后来把那个框整个删了:`LoopInput::Mail { owed }` 连同 `reply_owed` 一起退役,理由不是它太弱,而是**它根本判不了**——host 只知道"派下去过一件",判不出手上这条是不是那件的回答;而 Reaction 两头都有(请求在它自己的长会话里,消息就在眼前)。整批一个布尔的问题因此不复存在:没有布尔了。规则搬进 `reaction.md`,**并且明写"数一数来了几件"**——一条消息夹三件是常态,没有任何东西替你记账,而最容易被剪的正是离当下话题最远的那件。见 `docs/arch/agents.md` 的 hand-down 一节。**同样未复测。**
...
## 32 · 每半分钟丢掉几个字,而且丢得悄无声息 · ✅ **已修 `fix/voice-input-segmentation`,回放实测已做;2026-09-10 真人口述复测已做(见本条末尾),并从中接出一条新修法,那条的效果尚未实测**

**症状。** 2026-09-09 中午,老板用语音说话时报了两件事:①**分段呈现非常凌乱**;②**实时识别转最终确认时,内容会先消失一会,再突然出现最终结果——而且有时最终内容直接丢失,根本没有输入成功**。他当场只要求记下来提醒,没有授权修复;当天下午定位并修掉。

**实测方式(这条是量出来的,不是读代码读出来的)。** 当天的麦克风音频按分钟存着,所以能把**同一段声音**按实时速率重新喂给**同一个识别器**(`speaker_info` 同样打开),把它回送的每一帧 partial/final 录下来,再拿**旧实现和新实现跑同一份帧**。回放了三段共 **164 秒**:`data/memory/raw/audio/2026-09-09/04/{40,42,43,44}.wav`。

**结果:旧缓冲 37 条消息,新缓冲 36 条,5 处不一样——每一处旧的都在吞字或吐残渣。**(两边用的都是当时那套标点切句规则,单独隔离缓冲这一个变量;切句规则本身后来也换掉了,见本条末尾。)
...
| 04:40 | `。` | `稍微转两。` | **整句只剩一个句号** |
| 04:42 | `，然后。` | `然后。` | 多一个句首逗号 |
| 04:43 | `我。` + `就这个语音输入的时候…` | `就这个语音输入的时候…` | 多一条 `我。` |
| 04:44 | `时候感觉它，我不知道…` | `而且有时候感觉它，我不知道…` | 少三个字 |

**约每 33 秒说话就坏一处。** 当天真机日志里同一位置也坏了,只是断点略有不同(`我不知道它第二步…` 当时丢的是 `我不`)。

**注意:短消息本身不是证据。** 当天日志里那些 1–2 字的消息(`嗯。`、`啊？`、`对吧？`)大多是**真的独立短句**——新旧实现都照样产出。能算数的只有上表这种**新旧不一致**的地方。

**机制。** `foundation/segment.rs` 的缓冲是 `locked + partial`,而 `dispatched` 是一个**字符下标**,记「已经发出去多少字符」。回放录到的帧证实了它为什么必然错:**识别器的 partial 是整段累积的**(04:44 那一段,一个 utterance 跨了 23 秒、104 个字),最后一帧 final 是对**同一段**的润色重写——它删掉了 partial 里的 `？` 和两个 `，`,又补上两个新标点。于是 `locked.push_str(final)` 之后,那个按旧串数出来的下标就落在了新串的错误位置上:

- 新串在已发区**变短** → 下标越过了应发的起点,`而且有` 直接没了;越界时**整句 final 一个字都不发**,而且下标停在缓冲区之外,**接下来每句都被啃掉句首**。
- 新串**变长** → 下标切在词中间,残渣自成一条消息,`。` 那条就是这么来的。

**第二层:预览。** 旧实现的 `interim` 发的是识别器那份**累积 partial**,而句子落地时 `append` 会清掉 interim——清掉之后,**要等识别器下一帧到达才重新出现**。04:44 那段量到的帧间隔中位数 356ms、p90 926ms,于是每落一条消息,预览就空一下:实测 10 条里 0.03s–**1.90s**,三条 ≥1.4s。而它回来时显示的又是**整段累积 partial**,里面**包含刚刚已经落成消息的那几句**。人看到的就是"实时内容 → 消失一会 → 又突然冒出来(还是那段话)"。

**先前把这归给 `INTERIM_STALE_AFTER = 3s` 那个定时器,是错的。** 164 秒里帧间隔最大 4.01s,但那次的 interim 早已被 `append` 清空,定时器无事可做;其余空档都不到 3 秒。**这三段里它一次都没触发过。** 它照样删了——一个只可能早于它所预览的那条消息而触发的定时器,本来就没有存在的理由——但它不是老板看到的那个现象的原因。

**修法。** 两处改的都是"谁有资格结束一件事":

- **发出即从缓冲区移除,不再用下标越过它。** `Segmenter` 现在记住"这一句已经说出去的那段文本",新一版到达时跟它做**前缀对齐**(编辑距离最小的前缀即已说部分),后面才是要发的。标点增删、语气词删除、同音改写都能对上。
- **预览发的是切句器自己的未发出尾巴,并且在同一轮循环里立刻重发。** 消息落地后不再等下一帧 partial,空窗消失;预览里也不会再出现已经落成消息的句子。定时器删除,只剩两个事件能结束预览:句子落地,或识别流结束。

**顺带一个观测缺口(未修)。** STT 帧哪里都不落盘——`channel_log` 只记落定的入站行,`/api/channels` 是 presence 不留历史。这次能量出上表,靠的是**音频恰好按分钟存着**可以回放;要直接从日志里读出 partial/final 序列,还得让 STT 帧留下来。

**涉及 journey:** [15](15-talk-over-the-agent.md)、[31](31-hear-the-room.md),以及所有语音入口。

**同一分支上还换了切句的单位。** 老板 review 完上面的回放后,指出"断句语义上不够整体";按同一份帧把几种切法都跑出来给他挑,定下**一句话 = 识别器判定的一个 utterance**,不再按标点切。这是设计变更,写在 [`surfaces.md`](../arch/surfaces.md) 里,不是这条缺口的一部分——但它把同一段 164 秒从 36 条变成 15 条(中位 18 → 38 字),读这条记录时得知道最终上线的是后者。

**还欠什么。** 单测钉住三种改写形状、新的切句单位与预览的生命周期,`make test` 全绿,回放实测如上。**真人对着麦克风说一段带长停顿的话还没测**——回放复现不了"人停口后识别器迟迟不给 final"这种收尾停顿,而那正是预览寿命唯一还没被观测过的场景;新的切句单位在真机上是否会因为等端点而让上屏感觉变慢,也只有真人说话能判断。

### 2026-09-10 复测:真人口述测到了,上一条欠的那一项还清了,并且暴露了下一层

老板当天中午对着麦克风做了一段 **5 分钟的连续交代**(13:45:32–13:50:16,买鞋任务 + 截图留证),外加 14:02 的两句补充。这就是上面欠的"真人带长停顿的口述"。数据全在
`data/memory/raw/audio/2026-09-10/`(22 条语音消息 + 逐分钟 wav)与
`raw/sessions/24990c87019e/reaction.jsonl`。

**上一轮的修法成立。** 22 条消息中位 38 字,当天**一条 `嗯。`/`对吧？` 这类碎屑都没有**——对比 9/9 那次按标点切的 36 条 / 中位 18 字 / 下四分位 6 字。老板的原话是"比以前好,以前太零散了"。

**但 11/22 条是半截话。** 5 条以 `，` 结尾,6 条完全没有句尾标点,其中 4 条只有 2–14 字
(`就是`、`然后这个`、`就是倒不在乎说一定要追求特别`、`当然回到…我希望你也能够`)。**每一条后面都还有话**,以独立消息在 1.4–2.8 秒后送到。两个成因同时在开火:

- **96 字上限。** 13:50:00 那条**正好 96 字**且以 `？` 收尾(`snap_back` 从上限回退恰好落在标点)。撞上这个数约 1% 概率,所以 cap 确实在切。85+14=99、86+25=111 也过线。
- **800ms 端点本身。** 57+12=69 字够不到 96,只能是人在句子中间犹豫超过 800ms,服务端 VAD 就地收口。

**下游代价量出来了:23 次 `hi_say` 有 7 次生成完又被 floor 扔掉**(把 `reaction.jsonl` 的 `hi_say` 参数逐条比对 `raw/text/` 的落地消息得出),5 次就在这段交代里。而 13:52:42 那个 turn 里 cognition 的原话是"**已实际 cancel,暂停新的手机操作,等用户把要求说完**"——照着没说完的话派出去的活,回复能收回,活收不回(这是 [#28](gaps.md) 的形状,只是量级从 0.8 秒变成 10–55 秒的思考停顿)。

**修法(`fix/calm-speech-segmentation`)。** 把"识别器收口"和"人说完了"分开:收口的那句话若**不以 `。？！` 结尾**就不发,攥着等下半句,合并后一起走;真停在半句上则由 `hold`(3s,叠在 VAD 的 800ms 之上)兜底放出。`max_chars` 96 → 220,否则 cap 会把 hold 刚合并的重新切开。依据写在 [`surfaces.md`](../arch/surfaces.md):**早不再买到更快的回复,只买到白跑的生成和要撤回的活**,所以切句可以晚而完整。

**回放实测(2026-09-10,`tests/speech_segmentation_replay.rs`)。** 把 `05/45..50.wav`(221.7s)按实时速率重放进同一个识别器,录下 **358 帧(16 个 final、342 个 partial)**,再拿新旧两套策略跑**同一份帧**:

| | 条数 | 中位 | 最长 | 半截话 |
|---|---|---|---|---|
| 旧(endpoint = 一条消息,cap 96) | 20 | 39 字 | 113 | **10 / 20** |
| 新(完句才发,hold 3s,cap 220) | **11** | **45 字** | 289 | **1 / 11** |

`hold` 扫描(cap 220 不变):

| hold | 0.15s(=不持有) | 1s | **2s** | 3s | 4s | 6s | 8s |
|---|---|---|---|---|---|---|---|
| 条数 | 16 | 16 | **11** | 11 | 11 | 11 | 11 |
| 半截 | 6 | 6 | **1** | 1 | 1 | 1 | 1 |
| **末条延迟** | 0.15s | 0.15s | **0.15s** | 0.15s | 0.15s | 0.15s | 0.15s |

**最后那一行是这次最值钱的数字。** "等待在该回话的时刻不花钱"以前是个论证,现在是量出来的:每个取值下**这一轮最后一条消息都在同一个 0.15 秒落地**——因为人停口那句必然以 `。？！` 收尾,根本不进 hold。曲线在 2s 就平了,`hold` 取 3s 是**故意多留一格**:回放拼接的分钟 wav 缺了三成墙钟(见 [#33](gaps.md)),**抹掉了真实存在的停顿**,所以它报出的拐点是真人所需拐点的下限。

**`settle` 扫描(中间态)则表明没有中间态可选了:** 0.15s / 0.4s / 0.7s / 1.0s / 1.5s **全部是 11 条**,只有到 2.0s 才掉到 10 条、代价 2.10 秒末条延迟。抬 `settle` 换不到更少的消息——缓冲改动已经把它以前做的合并做掉了。留 0.15s。

**同一份帧还逼出了一次简化,而且简化让结果更好。** 老板问"为什么还要关心编辑距离"——`unemitted` 那 45 行,是为了在守卫从**还会被改写的 partial** 上切了一刀之后,把随后到达的 final 跟已发出的那截对齐。可 partial 的正当用途只有两个:**给人看的预览**,和**证明有人还在说**(barge-in)。让它的文本有机会变成消息,才是那 45 行存在的原因。

改成**只切已定稿文本**之后:`unemitted`、`is_significant`、`said`、`pending` 全删,缓冲从四个字符串变两个;`Cut.committed` 没有了(tail 按构造就全是已定稿的);`max_segment` 那个守卫也跟着走——它标称管的是"识别器再也不收口",而那种情况下缓冲里一个字都没有,它根本无事可做,属于"described and absent"。

代价里唯一真实的一条:识别器彻底不收口时,东西到不了 agent。它换来的是**预览仍然在把人自己的话显示出来**(不是静默),外加流结束时上游会 flush 最后一条 final。

**而回放显示这一刀让结果更好,不是更差:**半截话 2 → **1**,新旧的**文本分歧 2 处 → 0 处**(再没有任何东西是从 partial 发出去的,自然不会出现"旧的拿到 partial 版、新的拿到 final 版")。同时 `hold` 需要的值从 3s 降到 2s 拐点(仍取 3s 多留一格),`settle` 的中间态整段消失。

**修法(`fix/calm-speech-segmentation`)最终三个值:`hold` 3s、`settle` 150ms、`max_chars` 220。** 规则两条:定稿文本以 `。？！` 收尾就等 `settle` 发出,否则攥着等到静音满 `hold`;外加一个 `max_chars` 连读兜底。**`max_chars` 不是消息长度上限**——说完的整句照发(回放里最长 289 字),它兜的是"没人停口的连读"。

**还欠什么(这条)。** 回放有一处系统性偏差:分钟 wav 缺了三成时间线(见 [#33](gaps.md)),拼接会**抹掉真实存在的停顿**,而停顿正是被测变量——偏向是**不利于新策略**的(更少停顿 → 更长连读 → 更多 cap 硬切),所以上面几张表是下限而非估计。**无偏的答案要等 STT 帧真的落盘之后的一次真人对话。**

还有一个**没测到的形状**:289 字一条的消息在真机上读起来是否嫌长,回放答不了——它只数得出条数和字数,答不出"这样一条喂给 Reaction 合不合适"。要是嫌长,该动的是 `max_chars`。

**顺带两个观测缺口(未修)。**
- **STT 帧仍然哪里都不落盘。** 9/9 记过一次,这次又因为同一个原因量不出效果。—— ✅ **已修 `fix/mic-audit-trail`:每一帧 partial/final 落到 `audio/<date>/frames.jsonl`,跟着当天字节一起淡出。** 落定的消息说的是“决定了什么”,只有帧说得出“是从什么里决定的”。下一次这类问题不用回放、也不用真人配合。
- **声纹在同一段独白里掉了两次:** 13:47:52 和 14:02:06 被标成 `⟨voice: unfamiliar⟩`,前后都是同一个人连续说话。不属于断句,单独记着。

---

## 33 · 麦克风的分钟存档会把自己覆盖掉,于是"原始记录"缺了三成 · ✅ **已修 `fix/mic-audit-trail`,未复测**

**症状。** 存下来的 mic 音频**不是当时听到的全部**。2026-09-10 那段 5 分钟口述里,`data/memory/raw/audio/2026-09-10/05/47.wav` **只有 14.1 秒**——而那一分钟人一直在说话,光是 13:47:02 落地的 `嗯,截图,我希望你完整保留哈…` 一条就 93 字,按语速已经约 20 秒。六个文件合计 **221.7 秒**,却跨了约 **320 秒**墙钟。

**机制,两行代码合起来的。** 分钟存档走
[`flush_mic_minute`](../../src/foundation/server/audio.rs) → `media::store_blob` →
`File::create`,而 `File::create` 是**截断**写;文件名是
[`media_rel_path`](../../src/mind/memory/layout.rs) 的 `InputStream` 分支,`<HH>/<MM>.wav`,**一分钟一个固定名字**。

而 `flush_mic_minute` 有两个调用点:分钟翻转时,**以及每次这条 socket 关闭时**。mic 上行 socket 是自愈的([`audioStreamer.ts`](../../src/appearance/web/src/lib/audioStreamer.ts) 断了就重连),服务端 handler 也会在 STT session 结束时退出重来。**于是同一分钟内只要重连过一次,后半段就把前半段整个盖掉。**

**为什么它比"丢一段录音"更贵。** 这份音频是**唯一**能事后回放的原始信号:STT 帧不落盘([#32](gaps.md) 记了两次),`channel_log` 只留落定的入站行。所以每一次"断句/识别到底发生了什么"的复盘都要靠它,而它是残缺的——9/9 那次回放的"164 秒"多半也少了一块。声纹按 `[start_ms, end_ms]` 切说话人音频走的是内存 timeline,不受影响;受影响的是**事后**的一切。

**要注意它不影响对比的公平性。** 新旧策略跑的是**同一份帧**,所以两者之差仍然可信;残缺影响的是"这份帧的停顿分布像不像真人",不是"哪个策略更好"。而且它的偏向是**不利于新策略的**(停顿被拼接掉 → 更长的连读 → 更多 cap 硬切),所以回放里新策略的优势是下限。

**修法。** 走的是②的一个变体:`MediaSlot::InputStream` 从 `<HH>/<MM>.<ext>` 改成 **`<HH>/stream/<MM>-<SS>.<ext>`**,按片段**起始秒**命名。重连就是新起一个文件;两路同时开麦也天然不撞。**没选①(追加)**,因为路由 `WS /api/in/audio/stream` 没有单例保护,两路音频会被交织进同一个文件——审计日志里"噪声"比"缺口"更坏。改名不碰读者:仓库里**没有任何东西按路径读这些文件**(`parse_ref` 明确拒绝这个形状)。

**顺带修掉的是同一个想法的第二处走样。** `media::store_artifact` 早就从另一头撞上过重名,长出了自己的一套 `try_exists` 再创建的后缀循环——同一个想法说了两遍,而且第二遍有竞态。现在只在 `media::claim` 说一次,用 `create_new` 原子地说:两个写入者抢同一个名字会各自解决,而不是其中一个悄悄赢。**这才是承重的那一半**——网格让撞名变罕见,`claim` 让它无害。

**改的时候差点漏出一个更贵的洞。** 遗忘扫描 `decay::input_grid` 只读 `<HH>/` 下的**直接文件**;流媒体一搬进 `stream/`,它就再也看不见了——**每一分钟的麦克风和摄像头都会被永久保留**,而周围一切照常淡出,静悄悄地。`sum_full_bytes` 和 `drop_full_bytes` 本来就是递归的,只有 `input_grid` 是平的。另外 `claim` 给出的 `-2` 后缀会让 `parse_minute_file` 解析失败,同样等于漏扫。两个都修了,都有单测。
