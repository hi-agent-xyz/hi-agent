...
这一轮**修好的**:#2 #4(大部分)#5 #8 #16 #18,以及 #6 #7 #14 #15 的一半。
这一轮**新发现的**:**#20**(turn 期间派不出 worker —— **已在本轮修掉并现场复测**)· **#21**(坏仪表让 app 永远不来解除 Pause,🔴,= #13 升级)· #23(交付路径的调研没预算)· #24(Cognition 对在场瞎猜)· #25(Cognition 的失败路径不认三分法)· #22(🟢,#20 的后果)。

**净判断:对话与记忆这一层明显变好了;剩下的问题集中在"把活派出去"和"从故障里爬回来"两处。** 两条 🔴 各有一句话的根:**#20 是 `create_worker` 的契约与实现自相矛盾**(要求调用者先停下,交接才完成);**#21 是 `remaining=0` 这个恒假读数被接成了恢复链上唯一的开关**。两条都在宿主侧,引导侧改不动。
...
**复测 2026-08-05 · `a05b734` — 通过,而且比要求的更强:那句话跨进程重启活下来了。** 09:57:37 问"今天有什么大新闻?",当时 scene 正卡在 `Pause`(见 #21),这句话**一个 turn 都没驱动**就被扣住了。10:02:45 我杀掉进程重启。10:05:17,新进程的第一句话是:
...
- ✅ **恢复收 view 仍然对**:02:05:40 服务恢复后 `id=vendor-outage op=Dismiss`。但要注意它是**靠重启才恢复的**,不是自己好的——见 #21。
...
**复测 2026-08-05 · `a05b734` — 确认,而且这条不再是"只影响可观测性"了。**

最干净的一次对照:**同一分钟内,端点说枯竭,agent 正在正常干活。**

```
10:11  {"out_of_energy":true,"resets_in":"大约 646 小时后"}   ← 27 天后
10:05  agent 正常回话;10:34 交付了七条新闻 + 视图上屏
```

broker 自己的读数也一样:`energy refreshed tier=standard remaining=0 total=100000`,而这期间 turn 一个接一个成功。`reconcile()` 的逻辑是 `set(remaining <= 0)`,所以只要 broker 报 `remaining=0`,`is_out()` 就恒为真——**在这个账号上它就没有过第二种取值**。

**升级的原因:`a05b734` 把这个坏掉的仪表接成了承重墙。** `Disposition::Pause` 的**唯一**解除条件是 `!energy_state::is_out()`。仪表恒真 ⇒ 暂停永不解除。详见 **#21**——本轮它把 agent 卡死了 10 小时半。**这条不再是 🟡,而是 #21 的根因。**
...
## 21 · 解除 `Pause` 的那个仪表是坏的,于是 app 永远不来救 · 卡死 10 小时半 🔴 · **本条是 [#13](gaps.md) 的升级,不是独立的设计分歧**

**症状。** 一次真实的 402 之后,agent **再也不说话了**。人发的消息收得到、记得下,但**一个 turn 都不驱动**。屏上挂着一块 outage view,文字通道一个字没有。10 小时半之后我重启进程——**第一个 turn 就成功了**。额度一直是好的。

**证据(2026-08-04/05,`a05b734`)。**

```
14:50:19–15:28:30 UTC   真实 402 × 53 → Disposition::Pause,SceneGate::Hold
15:28 – 02:04 UTC       10h36m,零 turn、零 402(它根本没再试过)
01:57:37 UTC            我问"今天有什么大新闻?" → 只有 `channel in`,没有任何 turn
02:02:45 UTC            我 kill + 重启
02:05:48 UTC            turn 成功,正常回话
```

**关键的一句:重启后它一次就通了。**所以 15:28 之后的某个时刻额度就已经回来了,而**它从未去看过一眼**。

**机制。** `Pause` 的解除条件只有一个([`reactor/mod.rs:1425`](../../src/body/reactor/mod.rs)):
...
注释写着 *"A pause is released by whoever owned the reason, never by a deadline here. In managed mode that is the broker's 60s energy poll."* ——设计是自洽的。**问题是那个 poll 报的数是假的**:`reconcile()` 就是 `set(remaining <= 0)`,而这个账号的 broker **恒报 `remaining=0`**,包括此刻它正在正常干活的时候(见 #13 复测:10:11 端点说枯竭,10:05 和 10:34 它在正常交付)。`is_out()` 恒真 ⇒ **`Pause` 永不解除**。

**这是 #13 从"仪表读数难看"升级成"承重墙塌了"。** `a05b734` 的推理链完全成立——*"Retrying cannot help, and each attempt costs a subprocess spawn: run-b made 487 of them"*——它治好了 #14 描述的那个浪费。但它把恢复的唯一钥匙交给了一个**这份清单自己在 #13 里记了两轮"不可信"的仪表**。

**架构上没有分歧,别把这条读成"agent 该自己重试"。** 402 掀翻的是**整个 app 的状态**,由 app 负责刷新状态并把 agent 放回去跑——这正是 `a05b734` 有意确立的分工,`Retry` 被从这条路上拿掉是对的。

**这条记的是:app 那一侧的活没干成,因为它唯一的输入在说谎。** 恢复机制本身是齐的(broker 每 60s poll → `reconcile` → 清 gate → 排队的信重新灌进来),链条上**只有一个环坏了**:`reconcile()` = `set(remaining <= 0)`,而这个账号的 broker **恒报 `remaining=0`**,包括它正在正常服务的此刻。恒真的 `is_out()` ⇒ 恢复条件永远不成立 ⇒ app 永远不来救。

**所以修的地方是 #13,不是这里的重试策略。** 把仪表修对(或让 `reconcile` 不再从一个已知会撒谎的字段推断),#21 自动消失。

**唯一值得单独留一句的是:这条恢复链只有一个输入,而且没有任何东西能发现这个输入在骗人。** 一个只能由外部信号解除的停机状态,可靠性上限就是那个信号的可靠性——本轮它的可靠性是 0。这是对**状态刷新**的稳健性提醒,不是对"agent 该不该自己重试"的翻案。

**对比一下换掉的是什么(供定位严重性,不是主张回退):** 在 `a05b734` 之前,402 走通用 `Retry`,退避重试、迟早自己好——坏仪表当时不致命。现在同一块坏仪表变成了承重墙。

**另一个独立的洞:被卡住的时候人是不知道的。** 那 10 小时半里,文字通道零输出(#6 的未修那一半),屏上那块 outage view 也一直没撤——**从人的角度看,它和"死了"没有区别**。

**涉及。** 所有 journey 的失败路径 · [05](05-news-and-watch.md)(重启不丢盯:这次是"重启才活")。
...
- **`Pause` 之后,"没反应"不等于"没额度"。** 本轮差点把一次 `Pause` 卡死误判成配额耗尽而中止测试(端点、broker 读数、`resets_in: 646 小时` 全都指向"没钱了")。**判据是重启进程再发一句话**——`VendorState` 不持久化,重启即清,一次就能把"真没额度"和"卡在 Pause"分开。见 #21。
