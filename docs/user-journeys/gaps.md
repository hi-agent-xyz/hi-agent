...
**顺带两个观测缺口(未修)。**
- **STT 帧仍然哪里都不落盘。** 9/9 记过一次,这次又因为同一个原因量不出效果。第三次遇到之前应该把它修了。
- **声纹在同一段独白里掉了两次:** 13:47:52 和 14:02:06 被标成 `⟨voice: unfamiliar⟩`,前后都是同一个人连续说话。不属于断句,单独记着。

---

## 33 · 麦克风的分钟存档会把自己覆盖掉,于是"原始记录"缺了三成 🔴 **未修**
...
**修法(未做)。** 三个方向,没定:①同一分钟改成追加(读回已有文件、拼 PCM、重写 header);②文件名带上 handler 序号,一分钟允许多个片段;③干脆不按墙钟分钟切,按 session 切。**②最省事且不用读回,③最干净但改了 `MediaSlot::InputStream` 的约定。**
