...
> **像人一样攒能力的三条反射(22–24)**:不是塞一个大知识库,而是按 decay-rate 配上**获取反射** + 元心态。研究反射(开工前别拿陈货)→ 批判反射(收工前别交 dumb)→ 技能沉淀(把贵经历存成起点,且会重新核当下)。研究/批判主体是 soft-guidance;技能沉淀要一个放技能的 `skills/` 文件夹 + reflection 多策展一类。
...
- [24 · 第二次剪快得多,而且没用陈货(技能沉淀:难活变顺手流程,且重新核当下)](24-skill-improves-and-refreshes.md) — 一次又查又试又翻车的贵经历沉成 `skills/` 一条笔记,第二次**从 bar 起步**、明显快;但**技能=起点非真理**:durable 半(我怎么干)稳用、transient 半(当下什么好/哪个工具)每次被研究反射([22](22-research-before-stale-answer.md))**重核**——正面解 [11](11-china-tax.md) 的"技能别把旧数字焊死"。门槛=难+会再来+干成了;reflection 策展(照它策展 facets/drive)。实测:worker 自己写了一条技能(contribute 路径通过);reuse/策展/transient 标记待复测。
...
- [30 · 画张图,再改一版(画出来就是附件,拿 `att:` id)](30-make-a-picture.md) — 四个生成任务(text/image-to-image、text/image-to-video)从"声明了但没接线"到真跑通:**模型由 agent 挑**(工具描述现列当下可达的模型 + 最好/最快/最便宜),旋钮咽不下就报错点名换谁能办、**绝不静默丢弃或换模型**;产物进附件库(**不褪色**,区别于 raw 的相机帧,按内容寻址),回的是 `att:` id —— 相机拍的 / 别人递的 / 自己刚画的走同一个参数,画完直接能上屏;视频不占线,几分钟后以**一条消息**带 ref 送回。填上 [12](12-play-with-child.md) 实测记的"图是代码画的、非图像模型"。**图像那条已实测出图**(2048×2048 真图落 drive、ref 可回喂、不能编辑的模型如实拒绝);gpt-image-2 只有单测、视频两条未跑;agent 自主派单那段被本机 Reaction 不开口挡住,待复测。
...
- [37 · 单手拿着它:房间里什么按钮都没有,从右边缘划一下,面板就进来](37-hold-it-in-one-hand.md) — 房间里**一个按钮都没有**:角上那排六个控制删掉了,house 的三样东西(对话、views、频道控制)合成**一块面板**,在一根轴上滑进滑出——右边缘进,左边缘出,手机上只有「房间」和「整屏」两档(390px 分不出两栏)。真的拖:一比一跟手,拖不够弹回来,拖过头或者甩一下才走。面板 head 上是 tab(`Messages`/`Views`)加四个频道按钮,**每个 stop、每个 tab 上都一样**——「每个频道随时一按就到」这条老规矩换成了更窄的一条:**没有频道躲在模式后面**。代价说清楚了:开麦克风从一下变成两下。iOS 键盘那条仍然只能靠 `visualViewport`。**手机形态在模拟里看过,真机未实测**——键盘、安全区、以及最要紧的「零按钮的房间真人找不找得到路」,模拟器都答不了。
- [38 · 一边看它摆上来的东西,一边说话:面板把 view 挤窄,而不是盖住](38-read-it-and-say-something-about-it.md) — 同一根轴在桌面上多一档中间的:面板 420px 靠右,**view 排进剩下的 1092px 而不是被盖掉一块**。这是把 2026-08-17 杀掉的 rail 放了回来,而放回来的理由是它当初死在的那个词——rail 是**常驻**收租,中间档只在人自己把它拉出来的时候收,而且重排是**人点的**,不是 agent 摆东西摆出来的。桌面开机停在中间档(角上那个入口没了,一块全白的纸没有看得见的门),打字仍然直接开。**已建,无头里逐档验过并真的摆了一块 view 上去看重排,真人未用过。**
- [39 · 说出它的名字,它就问「放我进来吗?」:加一台远端设备,不用抄码](39-add-a-remote-agent.md) — 加设备的第三条路,也是唯一一条**从另一个房间就能走通**的:新设备说出 agent 的**名字**,人在 `reach` 页上点一下放行。两边显示同一个**六位数**——它不是密钥、不授权任何东西,只回答「同时有两台在等,哪台是我手里这台」。接口不需要凭证(要凭证的话,需要它的人正好没有),所以**八台上限就是防洪**,第九台直接 `429`。**agent 不会被告知有人在等**:任何能通到 Cognition 的路都等于让陌生人支使别人的 agent 说话。顺带修掉了浏览器那条老毛病——session 过去只活在进程内存里,**core 一重启浏览器就被挡在外面**,而它手里那块 cookie 上写着三十天。现在 session 和凭证存在一起、用过一天就续,token 不换。**接口、`reach` 页、iOS 三屏都在真 core 上看过(含在渲染出来的页面里真的点了放行/拒绝),真机两台设备之间未实测。**
- [40 · 在任何 app 里点分享,选 hi agent](40-share-anything-to-the-agent.md) — 系统分享单成为 agent 的入口:照片、文件、链接、选中的字,两下点击到对话里,而且**落地那一刻人就停在能接着说话的地方**——分享是把东西放到桌上,不是发完就走。所以**分享不带 note**:分享什么就是说了什么,那句旁白该他自己说。链接走"说"那条门(`/api/in/text`),不是存成一个要打开才知道是链接的文件。两边都**没有大小上限**了,因为收发两端都改成了流式——原来那个 50 MiB 也不是真上限,真上限是内存。iOS 上**扩展只排队、app 才发送**:分享扩展的 sheet 一关就被杀,改成排队之后队列还顺带成了重试缓冲(截图手势也搬了上去);Android 不需要这套,`ACTION_SEND` 直接就把 app 打开了。而把 app 唤到前台这一步是整件事最容易骗人的角落——**四种写法三种是死的、其中一种直接崩**,能用的只有 `EnvironmentValues().openURL` 打一条 Universal Link,于是顺带多了 Associated Domains 权限、一屏一个按钮、和一个 50ms 的 runloop 间隔。**一次都没跑过:三边都编译并通过测试,但没有人打开过一次分享单。**
- [41 · 首页这几摊事应该放在一起](41-arrange-my-work.md) — 十几张任务卡平铺在核心周围,没人是这么拿着自己的工作的:哪几件是一摊事只有本人知道,而且他心里的分法不是任何字段能推出来的。所以**分组归首页自己所有**——不是任务上的一个 `project:` 字段(那会为了一个视图把一条轴按给账本的所有读者),而是两份记录:`home/grouping.md` 是他说过的原话、散文、**代码一个字都不解析**,`home/groups.json` 是当前这一版排布、JSON、**只能通过校验过的工具写**。分工按读者划:模型读散文,渲染器读结构。**代码一条都不推断**——被删掉的那个主题层就是推出来的,`systems` 说的是任务碰了什么,于是照片走飞书的生日 PPT 被画进了 "feishu"。改法是说一句话,而且**先落原话再改排布**,因为文件才是让这句话对下个月新建的任务仍然生效的东西。笔分三处:Reaction 只转话,Cognition 当轮把原话落进 `grouping.md`,排布交给 `task-manager` —— 一次看全部在办行的判断是差事,而「同时只有一个」本来就是它的纪律。实测把闭环整条走通了一遍(一句话进去,五分钟后 KTV 组出现在屏幕上),并且**顺手抓到一个更重的 bug**:中文名字的 worker 因为 `X-HI-Session-Slug` 装不下非 ASCII,在服务端根本没有身份 —— 连报告都发不回去。
- [42 · 在任务上直接回一句](42-answer-on-the-task.md) — 一张卡写着 *等你处理*,答案是一句话,而回这句话的地方原来是对话——他得先把是哪一件重新说一遍,梯子上的某一层还得猜他说的是哪一行。现在**每个任务的面板底部有一个回复框**,回的这一句同时是两样东西:**对话里的一条消息**(`POST /api/in/text?task=`,入口给它带上"在哪一行说的"这个事实,交给 Cognition 时也是事实而不是 Reaction 的猜测),和 **store 在那一行上写的一条 `replied`**——它盖掉上面的 `waiting`,让 *等你处理* 当场消失,并且让回答活在行上,不会被重启吃掉;写进去之前先遮掉密钥,因为记录文件是用 shell 读得到的。**不是每个任务一个聊天**:agent 只在对话里说话,工作的回应是它写在记录里的行。实测(无模型)走通了入口、记录和真浏览器里的面板;整条梯子还没在有模型的实例上看过。
- [43 · 你说"就 A 吧",它马上接着往下做(它备了几条路,你走了哪条它就接哪条)](43-the-next-line-is-ready.md) — 人讨论时,提了个建议就已经想好:对方要是同意,我接着做什么。Reaction 没有"接着":它对一句话的任何反应都从一轮生成开始,而一轮到第一句有 13.8s 的地板,提示词压不动。所以**在他开口前就定好,他开口时只做'认'**:`hi_say` 是这一刻的回复;在它之上,`hi_prepare(branches)` 一次交上一整组可能的方向(一道选择题,重交即整组替换),每个方向一个条件加一串 Reaction 自己的动作(`hi_say` / `hi_show` / `hi_send_message`),方向之间**互斥**、最多命中一个。他的话一到,jev(本机约 0.7s、概率跨调用可比)判它走向哪条,以及有没有**给那条加条件**(「对,不过……」不跑;「对,然后……」照跑,后半句交给紧接着的一轮);命中就按序执行、前一步没成后面就停,打字约 1s、说话约 2s,对比今天中位 29s。然后这句话连同"跑了什么"交给 Reaction 正常一轮 —— 它可以接着说、沉默,或者再备下一组。没命中就照常,**不加时延**;最坏就是今天的速度。话只能说那一刻为真的(交下去了可以说"我去弄",不能说"弄好了");一组分支属于**一件事**、等的是那件事而不是下一句:他岔开去说别的它留着,谈到这件事却没命中就用掉,报告、别的轮、重启都不作废(落盘),过时的由 Reaction 自己清,最多 4 件事,**没有计时器**(被打断**不**作废:打断是估算出来的,说完两秒内的快回答也算,插进来的话照样拿去比)。和 [34](34-a-step-ahead.md) 成对:34 省等活的时间,这条省等一轮的时间。一个开关,默认开;量的是命中后紧接着那一轮不得不收回的比例。**已建,未实测**。
- [44 · 活儿还在做，做出来的东西已经能看了](44-see-it-while-it-is-being-made.md) — worker 做出一张能说明方向的图，写进展时把它**带在那一行上**（`hi_task_note(…, attach)`），几秒后首页那张卡下面就多一张图，点开是面板停在那一行、图整张打开、下面是那句原话；视频能播能拖。文件在那一刻被拷进 `attachments/`、按内容寻址、`immutable`——那一行的证据不会因为 worker 覆盖了文件而变；代码不从文件夹或句子里推断，挂出来的只有亲手带上的。浏览器放不了的编码拒掉并给转码命令。
