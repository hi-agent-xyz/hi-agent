...
- [40 · 在任何 app 里点分享,选 hi agent](40-share-anything-to-the-agent.md) — 系统分享单成为 agent 的入口:照片、文件、链接、选中的字,两下点击到对话里,而且**落地那一刻人就停在能接着说话的地方**——分享是把东西放到桌上,不是发完就走。所以**分享不带 note**:分享什么就是说了什么,那句旁白该他自己说。链接走"说"那条门(`/api/in/text`),不是存成一个要打开才知道是链接的文件。两边都**没有大小上限**了,因为收发两端都改成了流式——原来那个 50 MiB 也不是真上限,真上限是内存。iOS 上**扩展只排队、app 才发送**:分享扩展的 sheet 一关就被杀,改成排队之后新增的 entitlement 只剩一个 App Group,队列还顺带成了重试缓冲(截图手势也搬了上去);Android 不需要这套,`ACTION_SEND` 直接就把 app 打开了。**一次都没跑过:三边都编译并通过测试,但没有人打开过一次分享单。**
