...
- [18 · 我要传你点东西,怎么弄(摆出上传入口:拖拽区 + 二维码)](18-send-files-to-agent.md) — 最基础常用的一步:把东西递给它。优先**直接摆两个 view**(拖拽区 + 手机扫码上传页)而非口述选项;入口绑 scene。文件 = 递来的物件按引用,不走 vision 感官。行为靠 show 已有,carrier(上传端点 + 手机页 + 二维码)未建。
