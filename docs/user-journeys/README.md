...
- [37 · 单手拿着它:对话是推上来的一页,划一下就回去](37-hold-it-in-one-hand.md) — face 在手机上不再是"窄一点的桌面 face"。窗口那套(浮在角上的面板、按后面一下就关掉)在手机上三条前提全不成立:没有"后面"、没有富余的角、拇指是从下沿够上来的。所以**对话和 views 都变成推上来的一页**:整屏、从右边进、从左边缘拖回去,而且是**真的拖**——页一比一跟着手指,拖不够弹回来,拖过头或者甩一下才走(不是"识别到手势后播一段动画")。同时修掉一个静悄悄的缺陷:给 ~380px 菜单栏 popover 写的 `max-width: 420px` 把 393px 的 iPhone 也一起收窄了,控制键在唯一一个用手指按的 host 上被压到 32px——**宽度只答了一半的问题**,popover 会 hover,手机不会,所以"手机"是**窄 且 粗指针**。控制键回到 44px,并在页上变成页头那条 bar(键盘那个在那儿就是 `‹`)。另外 iOS 键盘那条只能靠 `visualViewport`:`dvh` 在 WebKit 上答不了(键盘是盖在没变过的 layout viewport 上的)。**已建,未实测**——模拟器里的键盘不是真键盘,安全区是 0,WebKit 自己的边缘也不在。
