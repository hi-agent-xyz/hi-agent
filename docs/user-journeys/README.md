...
- [43 · 你说"就 A 吧",它马上接着往下做(它备了几条路,你走了哪条它就接哪条)](43-the-next-line-is-ready.md) — 人讨论时,提了个建议就已经想好:对方要是同意,我接着做什么。Reaction 没有"接着":它对一句话的任何反应都从一轮生成开始,而一轮到第一句有 13.8s 的地板,提示词压不动。所以**在他开口前就定好,他开口时只做'认'**:`hi_say` 是这一刻的回复;在它之上,`hi_prepare(branches)` 一次交上一整组可能的方向(一道选择题,重交即整组替换),每个方向一个条件加一串 Reaction 自己的动作(`hi_say` / `hi_show` / `hi_send_message`),方向之间**互斥**、最多命中一个。他的下一句一到,jev(70–500ms、概率跨调用可比)判它走向哪条,以及有没有**给那条加条件**(「对,不过……」不跑;「对,然后……」照跑,后半句交给紧接着的一轮);命中就按序执行、前一步没成后面就停,打字约 1s、说话约 2s,对比今天中位 29s。然后这句话连同"跑了什么"交给 Reaction 正常一轮 —— 它可以接着说、沉默,或者再备下一组。没命中就照常,**不加时延**;最坏就是今天的速度。话只能说那一刻为真的(交下去了可以说"我去弄",不能说"弄好了");一组分支只对下一句有效,别的轮先跑、别的话先出去、重启,都作废,**没有计时器**(被打断**不**作废:打断是估算出来的,说完两秒内的快回答也算,插进来的话照样拿去比)。和 [34](34-a-step-ahead.md) 成对:34 省等活的时间,这条省等一轮的时间。一个开关,默认开;量的是命中后紧接着那一轮不得不收回的比例。**已建,未实测**。
