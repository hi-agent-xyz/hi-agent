...
2. **hi agent 到前台,停在对话上** → 底部一条"正在发送…",落地变"已发给 <agent 名字>"(多个就是"已发 3 个"),两秒自己收走。
...
- iOS 唤起宿主 app 用的是 responder chain 上的 `openURL:`。**这是唯一可能被 App Review 拒的地方**,单独放在 `OpenHost.swift`,删掉那个文件和它唯一的调用点,分享照样端到端能用,只是人要自己点开 app。到底留不留,没定。
...
_状态:**一次都没跑过。** iOS `xcodebuild` 过(扩展已嵌入并通过 embedded-binary 校验),Android `make android` 过(编译 + 单测),core 那半 `make test` 过 —— 但**没有人打开过一次分享单**:没有 drop 被排过队,没有排过的 drop 到过 core,responder chain 那下唤起既没看见成功也没看见失败。App Group 在 `CODE_SIGNING_ALLOWED=NO` 的构建里本来也不工作,模拟器要验证得按 [apple-ios.md](../platforms/apple-ios.md) 那样 ad-hoc 签名。_
