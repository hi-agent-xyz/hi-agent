...
| | The re-test |
|---|---|
| **What a reopened errand does *not* get back** — its inbox, and the exchange ring behind the Sessions page arrow | Its slug, thread, owner, subject, roster row and both mail directions are restored, so addressing works and `WorkerReport` still routes home. Two things do not. **Pending mail dies with the entry** (`unregister` drops the inbox) and never reached the thread, so the reopened session does not know it was sent anything — the count now rides the row and the boot pass tells the *sender*, which still holds the text and can judge whether it still applies. Unwatched: whether that post reads as actionable to Cognition or as noise. **The `registry::mail` ring is emptied by the restart** while the roster row it draws arrows between now survives — so an arrow can open an empty exchange for a pair that has been talking for an hour. Nothing is lost (both threads hold it, the wire log has every frame); the page just misleads. Rebuilding the ring from the frame logs at boot is the fix and is not built |
