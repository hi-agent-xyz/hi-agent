...
| **Going back, and coming back** | Two changes nothing automated reaches: going to the card already marked *live* clears the cursor instead of parking on it, and *close the view* now drops the cursor too (it used to clear the server's slot while the parked window kept rendering its own view). The return-to-live button is gone with them — the dot rides the views control. Tap the live tile in the band and confirm no dot appears on a later raise; then park on an older card, have something raised, and confirm the views control dots, the band's live card wears its pip, and the X clears the room from where you are |
