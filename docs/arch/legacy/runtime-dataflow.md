...
Build a reference implementation of the human-interface spec (a separate repo, not vendored here) — small enough to read in one sitting, faithful enough to actually talk to. Cognition is delegated to an ACP agent subprocess; hi-agent is the **human-interface layer around it**: the channels, the presence loop, the memory, and the session orchestration the spec implies.
