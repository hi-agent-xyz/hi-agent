# reaction / cognition / worker — the tempo split

> **Status: the split is the default and always on** (`split_enabled()` hardcoded true;
> the `HI_AGENT_REACTION_SPLIT` env flag is retired). The `feat/reaction-communicator`
> change makes the reaction a proper fast **communicator** and fixes the silence the split
> shipped with — see *Current state* below. The legacy agentic reaction path is dead code
> pending deletion.
>
> **The Contract section immediately below is authoritative (agreed 2026-07-12).** It
> supersedes two things in the historical notes: the reaction runs a **smart** model, not a
...
The reaction's speed was never about the model tier; it's about **zero round-trips**. So
the layers are cut on two independent lines:

- **Think vs. act.** Reaction and cognition both only *think* — read and reason, never
...
- **Bounded/instant vs. open/deep.** The reaction thinks *only over what it already holds*,
  in one generation. Cognition thinks *open-endedly* — pulls more memory, reasons in steps,
  takes seconds.

The reaction is a **smart** model precisely because its core skill is judging the edge of
...
| | **Reaction** — the mouth | **Cognition** — the mind | **Workers** — the hands |
|---|---|---|---|
...
There is **no agent↔agent link**. Both reaction and cognition are reactive ACP subprocesses
...
```
   human ─▶┌ reaction driver ─ select! over reaction_rx ─▶ session.prompt ─▶ say/show
 (/mcp) ─▶ │                         ▲ reaction_tx        │ delegate → cognition_tx
 surface   └────────────────────────────────────────────┘
   (/mcp) ▶┌ cognition driver ─ select! over cognition_rx + timers ─▶ session.prompt
 report    └──────── surface → reaction_tx · dispatch → worker registry
```

- **reaction→cognition** (`delegate`): after a turn, `cognition_tx.send(Delegate{…})`.
  Non-blocking — the reaction never awaits a reply.
...
cognition's "is this worth raising?" (memory + `proactivity.md`) then the reaction's "is now
...
- **Reaction warm-up** assembles the prepared working set (identity + `commitments.md` +
...
**Non-negotiable:** the reaction's first turn never waits on cognition. History — warm-up
once ran synchronously and blocked the first turn, which *was* the silent-turn bug. So the
reaction's cheap memory prep is the only synchronous part; cognition's warm is background.

### Staging

- **Stage 1 — prepared memory (this branch, `feat/reaction-prepared-memory`).** The reaction
  ...
  reaction turn (retained across warm turns via session memory). Directly fixes the "no
...
- **Stage 3b — full switchboard (DEFERRED).** The remaining, larger pieces: promoting
  ...
  non-blocking warm-up** (reaction prep sync + cheap; cognition warm strictly async, deciding
...
## Current state (reaction-communicator change)

What the split actually is today, correcting the historical design notes further down:

- **The reaction is a tools-light ACP session, not a direct Messages call.** The direct
...
- **Speed came from two real fixes, not from "no tool loop":** (1) the reaction now runs the
...
body, `warm_up`/`open_session`/`discard_reaction_session`/`drive_racing_inbound`/`DriveOutcome`,
...
heavyweight agentic ACP loop** — the persistent per-scene "reaction session".
...
reaction is a *whole small self* for the fast path (it perceives, decides whether/what
...
→ reaction = one fast LLM call → cognition = agentic loop).
...
> turned out to be the model + a hang-zone adapter, not the ACP envelope. The reaction now
> rides a tools-light ACP session (see *Current state*). The `speaking.md`-as-system-prompt
> goal is met via `reaction_system_prompt()` prepended to the session.

We evaluated reusing the ACP session with tools off + explicit prompts. **Rejected** —
ACP structurally cannot meet the reaction's two goals:
...
2. **reaction** makes one Messages call (`speaking.md` + presence + conversation tail +
...
- **`identity::reaction_system_prompt()`** — `speaking.md` inlined as the reaction's whole
  system prompt, under a frame naming it the *voice* and cognition the *hands*.
- **`body/reaction/voice.rs`** — glue: `config_from` (credential → Messages config, small
  slot, raw model) + `speak` + the `split_enabled` env gate. (`AgentConfig`'s fields are
  public, so no accessor was needed.)
- **`body/reaction/run_reaction_turn`** — the split turn path, branched at `run_turn`'s top:
...
- ~~**Rename** `SessionRole::Reaction` → `Cognition`~~ — **superseded.** The
  reaction-communicator change instead collapsed `ReactionVoice` into `Reaction` and deleted
  the agentic `Reaction`; cognition is a `Worker`. See *Current state* above.
- **Trivial-turn cost** — cognition is handed *every* human turn (even "thanks"); the
  reaction's reconciliation suppresses double-speak, but a cheap gate (reaction decides, or
...
1. **cognition persistence** — one warm persistent cognition session per scene (as the
   reaction session is today) vs. spun per turn. Default: keep it persistent/warm.
2. ~~**fast model** — default to the `small` slot~~ — **superseded by the Contract.** The
   reaction runs a **smart** model: its job is judging the edge of what it knows, which a
