...
An app does get bytes from the community's [cache](cache.md), which is not a third wire and
not an exception to that sentence: the app asks its core's own URL with its core's own cookie,
as it always did, and the edge in front of that URL answers some of those requests from bytes
the core already decided to put there. The app cannot tell which answered.
...
**Content does not ride the tunnel.** The relay's public egress measures 11 Mbps: a small
machine can carry anyone's conversation and nobody's photographs. So a core mirrors its
immutable bytes into a bucket, ahead of being asked for, and the edge in front of
`<handle>.hi-agent.xyz` answers those paths from the bucket for the owner's signed-in devices —
at the core's own URLs, against the same session cookie, with the core still the authority. A
core reached any other way is served by the core, as before.

The whole design — what may be mirrored, the signed session, the keys, what reaches the core —
is [cache.md](cache.md).
...
8. **A core's origin is its own.** One handle, one origin, root-relative throughout; no core
   ...
   is a delivery rule and was never a boundary. Where the community's edge answers on a
   core's origin, it answers only at that core's own paths, only with bytes that core
   mirrored, and only to a session that core signed ([cache.md](cache.md)).
