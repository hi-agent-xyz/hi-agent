...
| **`GET /api/tasks` is a list; `GET /api/tasks/<subject>` is the record** | A list that carries prose and timeline grows with the history behind the work rather than with how much work there is — on one live store 166 rows cost 2.0 MB, 95% of it closed work nothing on screen renders past a title and a date. A row carries what a surface *draws*: identity, clocks, the newest line a mind wrote and what its lines carried. A file a record merely *spells* is a name in a sentence and nothing more — resolving those against the folder was a second index of it, drawn from prose ([showing.md](showing.md)). The account — prose and whole timeline, 1.8 MB of that 2.0 — is read when a person opens one, which is the only place either is shown |
| **A row says which views a task made, and the host is what saw it made** | The row used to resolve every view name its prose spelled — four spellings, filtered against the views on disk — and a mention has no verb: a note that another task's page was on screen made that page this task's result. What makes a view a task's is now a `made` timeline line the store appends the first time a session serving the task renders it with `hi_review_view`, the same kind of witnessed line as `moved`. The row reads only those, newest first, still filtered against the views on disk. See [`home.md`](home.md) |
| **The newest line a mind wrote is a field (`latest`), not a walk** | Two derivations of it existed — the board's and the chart's — and a row has no timeline to walk. Computing it where the timeline is means one rule for what `moved` does not count as, in one place |
| **A review surface parks on its store's version; it does not ask on a clock** | A tick costs the same whether anything changed or not, and is *still* up to one period stale at the moment something does. `GET /api/tasks?since=<version>` answers when the ledger disagrees with that number: measured on one live store, a board sitting idle went from 2 MB every 8 s to 30 bytes every 25 s, and a record written on disk reached the screen in 582 ms instead of up to 8 s. Better on both axes at once, and the period stops being a number anyone picks |
| **What a store's version counts is the filesystem, plus this process's own writes** | A task record is kept by writing its `facet.md` — the agent does it with a shell, not through `write_task` — so there is no call to hang a notification off, which is the same conclusion `view_watch` reached for views. `patch_task` also bumps directly: it is the same counter, and it is the half that does not depend on the platform having given us a watcher |
| **The facets watcher subscribes to record directories, never recursively** | A task's folder is also where its work happened: 13,982 directories under one live store's `facets/`, 2,011 of them inside a single task that cloned a repo. `notify` takes one inotify watch per directory against a `max_user_watches` commonly 8,192, so a recursive watch would exhaust it on that store today and grow with whatever a worker clones next. Watching the three levels that can hold a `facet.md` is 302 watches there — bounded by how many records there are, not by what is inside them |
