...
| **A row says which views a task produced; it does not say what it mentions** | Four spellings reach a record (inline code, quoted, `views/x.jsx`, a `view_ref:` field) and a body is also full of backticked things that are not views. Matching them is loose and safe only because the answer is then filtered against the views actually on disk — which is what the home chart did client-side while it still had the prose. Resolving it where the prose is means the looseness costs nothing on the wire and needs no cap: the answer cannot be longer than the view index |
