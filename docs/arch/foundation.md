...
|---|---|---|
| **Reaction** | `hi_prepare` · `SendMessage`, and **no built-ins at all** | one verb for everything that reaches the person — what to say and show, prepared for the moment it is for and released by the floor ([prepared actions](agents.md#prepared-actions)) — and the ability to hand work down. It cannot read, fetch, or run anything — that is why it is fast |
...
| **Workers** | projected files and memory, shell, devices, web, build, brokered private capabilities, a task's record through `hi_task_note` · `hi_task_set`, and a view reviewer's verdict through `hi_view_verdict` | the job is here, so the surface is wide; ordinary drive files, including managed secret files, are locally usable |
