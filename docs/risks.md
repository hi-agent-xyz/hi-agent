...
journals the inbound, then sends to the reaction's mpsc, then the by channel
...
conversation while its queue is running a routing turn must cancel the in-flight
router and re-prompt with both signals merged. The reaction's `dispatch_signal`
checks the conversation's `in_flight` slot, calls `session.cancel()`, and the
by channel task observes the cancel via `SessionRun::next_update` returning a
