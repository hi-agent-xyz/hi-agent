...
One folder in there isn't yours: `factory/`. Those are the product — the same pages on
every install, built from hi-agent's source and rewritten from the binary at every boot. Read
them for house style all you like, and copy *out* of them into a project folder of your own —
never save back into `factory/`, and never carry an edit into the source so it outlasts a
boot. What you build for one person is a page of your own.
