...
/// `hi_share` — publish a saved view or an attachment as a page somebody outside can open,
/// or withdraw one.
///
/// **Publishing a view can fail, and that is the point of it.** A view that reads the agent's
...
/// what comes back broken. An attachment fetches nothing and needs no check. See
/// `docs/arch/sharing.md`.
///
...
/// than derived, because only the session that made the thing knows what it is *for*.
fn share_tool() -> Value {
    tool(
        "hi_share",
        "Publish one of your saved views, or a picture or clip you placed (its `att:` id), as \
         an ordinary web page, so a person or another agent can open it with a link. A view \
         is rendered first, with your API refused, exactly as a stranger will see it — if it \
         needs data it fetches while rendering, it will come back empty for them and this \
         refuses to publish it, telling you what it reached for. Only what that one thing \
         needs is ever served: a view's own module, its own folder and the attachments it \
         draws; an attachment's own bytes; nothing else of yours. Use `unlisted` for a link \
         that carries a key, when it should not be findable by guessing the name. Set `on` \
         to false to withdraw it — the link stops working, though a copy already cached at \
         the edge can outlive that by a minute. Show the person what it looks like before \
         you hand the link over.",
        json!({
            "type": "object",
            "properties": {
                "ref": { "type": "string", "description": "A view's ref, e.g. `project/name`, or an attachment's `att:` id." },
                ...
                "on": { "type": "boolean", "description": "Omit or true to publish. False withdraws something you published before." },
            },
            "required": ["ref"],
        }),
    )
...
async fn dispatch_tool(
    ...
    match name {
        ...
        "hi_share" => return do_share(data_dir, args).await,
        "hi_add_attachment" => {
            let served = slug
                .as_ref()
                .and_then(|id| registry::global().status(id))
...
/// `hi_share`: publish an attachment, or check a view the way a stranger will see it and
/// publish it if it survives that.
async fn do_share(data_dir: &std::path::Path, args: &Value) -> Value {
    use crate::foundation::server::share::{self, Refused, Shared};

    let Some(named) = args.get("ref").and_then(Value::as_str).map(str::trim).filter(|s| !s.is_empty())
    else {
        return tool_error("hi_share requires a `ref` — a view's ref, or an att: id");
    };
    let on = args.get("on").and_then(Value::as_bool).unwrap_or(true);

    if !on {
        return match share::close(data_dir, named).await {
            Ok(()) => tool_ok(&format!(
                "`{named}` is no longer published. A copy already cached at the edge can answer \
                 for up to a minute after this."
            )),
            Err(e) => tool_error(&format!("could not withdraw `{named}`: {e:#}")),
        };
    }
    ...
    match share::open(data_dir, named, unlisted, description).await {
        Ok(opened) => {
            ...
            let checked = match Shared::parse(named) {
                Ok(Shared::View(_)) => {
                    " — it rendered with your API refused, so what a stranger sees is what you \
                     just checked"
                }
                _ => ", on a page that is the picture or the clip itself",
            };
            tool_ok(&format!(
                "`{named}` is published{checked}. {where_it_is}.{keyed} Show the person what it \
                 looks like before you hand the link over."
            ))
        }
        Err(Refused::Name(why)) | Err(Refused::Unknown(why)) => tool_error(&why),
        Err(Refused::Check(reasons)) => tool_error(&format!(
            "`{named}` cannot be published as it is:\n- {}\n\nThis is not a broken \
        ...
        Err(Refused::Broken(why)) => tool_error(&format!("could not check `{named}`: {why}")),
    }
}
...
mod name_tests {
    ...
    async fn name_person_renames_a_cluster_to_the_name() {
        ...
        people_vectors::enroll(dir.path(), "ff32ce3w", people_vectors::Modality::Face, &[1.0, 0.0], b"m", "jpg", None)
            .await
            .unwrap();
    ...
    async fn name_person_renames_a_voice_cluster_too() {
        ...
        people_vectors::enroll(dir.path(), "ab12cd34", people_vectors::Modality::Voice, &[1.0, 0.0], b"m", "wav", None)
            .await
            .unwrap();
        let r = reflection_name_person(
            dir.path(),
    ...
    async fn merge_people_ties_a_voice_id_to_a_named_face() {
        ...
        people_vectors::enroll(dir.path(), "赵力", people_vectors::Modality::Face, &[1.0, 0.0], b"m", "jpg", None)
            .await
            .unwrap();
        people_vectors::enroll(dir.path(), "ab12cd34", people_vectors::Modality::Voice, &[0.0, 1.0], b"m", "wav", None)
            .await
            .unwrap();
...
mod surface_tests {
    ...
    fn brokered_http_is_worker_only() {
        assert!(names(Some("worker")).contains(&"hi_http_request".to_string()));
        for role in [Some("reaction"), Some("cognition"), Some("reflection"), None] {
            assert!(
                !names(role).contains(&"hi_http_request".to_string()),
                "{role:?} must not be able to spend a stored credential"
            );
        }
    }
