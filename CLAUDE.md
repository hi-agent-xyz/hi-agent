...
**Be specific, not literary.** Name the function, the file, and the failing condition — "nothing calls `take_pending(id)`, so messages sit in the inbox forever", not "it needs a drain". Metaphor is fine after the mechanism, never instead of it. And keep "the design says X" clearly separate from "the code currently does X". **Names inside the product are held to this harder, because they reach a person**: whatever the agent reads in a prompt it may repeat on screen, and hi-agent is nobody's household word, so a coined metaphor is one nobody can decode. What a thing is called is what it literally is, and a session is titled for what it is doing, verb first — `skills/` holds *skills*, and a session writing one up reads 「总结技能：…」. It was "the workshop" until 2026-09-22, and a card titled 「工坊：…」 was unreadable to the person it was on.
...
2. **Bundled desktop app** — `make dmg` builds the hermetic macOS `.dmg`; `make app` wraps the dev binary in a minimal ad-hoc-signed `.app` for local mic/camera testing. `make exe` / `make installer` are the Windows pair (both now run on Windows in CI as well as cross-building on the Mac mini; the installer they produce has never been installed). No Tauri and no Electron: the binary is its own shell.
...
`app/windows` is the desktop's other client, and the **first one written in the target shape**: a standalone .NET/WinUI 3 build that owns the process and runs `hi-agent.exe` as its supervised child. That costs nothing extra because Windows never had a shell to migrate — every macOS-native crate is `cfg`-gated and `main.rs` already routes a non-macOS start to the plain server path. It is **built but never run**: no host this repo is developed from has a Windows machine or a .NET SDK, so CI on `windows-latest` is the only compiler it has ever met — and as of 2026-09-20 it gets all the way to a 67 MB installer carrying both halves. **No window has ever appeared**, and a runner with no desktop session cannot make one. See [docs/platforms/windows.md](docs/platforms/windows.md).
...
- **Mic capture → shell** (resolves the one open item): keeps the engine 100% TCC-free rather than dragging a Microphone grant into it. `cpal`-in-engine was the only capability that could have stayed; the permission story tips it to the shell. **That reasoning is TCC's, so it is macOS's** — cpal is one portable dependency with no per-OS branch, the same shape as ffmpeg, and where an OS has no per-app mic grant to hold, nothing tips and the engine opens the mic itself. It does on Windows today.
...
**On Windows the shell already owns the process**, because there was nothing to flip: `app/windows` starts `hi-agent.exe` as a child, passes it `--port` and `--data-dir`, and holds it in a job object so it cannot outlive the shell. It serves no mechanism calls — nothing dials that seam yet — so what it demonstrates is the ownership arrangement. The one OS capability that does not wait for the seam is the **attention gesture's listening half**: the key tap (`WH_KEYBOARD_LL` on the right Ctrl) and the mic (cpal/WASAPI) are both in the engine, so press-and-hold listens on Windows and the tray draws it off `GET /api/listening`. The tap being in the engine is a **named loan** — no grant gates it there, so it needs no shell, but it is Win32 in a crate that is meant to hold none; `docs/arch/mechanisms.md` Open 4 is the fork. The single-tap and double-tap legs are recognized and land on nothing, because raising a window and grabbing a screen are the shell's. It does have the *other* half of the shell's job: `Views/SettingsWindow.xaml` is the native-presentational surface in WinUI, a client of the same loopback config API, with macOS's four panes and none of its own. It compiles, links and publishes on CI's `windows-latest`, and has never been looked at: whether `GET /api/settings` reaches a Windows client at all is still the first question it can answer.

**Phase 2 on macOS — flipping process ownership to Swift — is not started, but it got smaller.** The seam it needs is built — `WS /api/mechanisms` ([foundation/server/mechanisms.rs](src/foundation/server/mechanisms.rs)) carries calls *outward* — and **nothing dials it or calls it**: no shell serves mechanisms, and the perceive/act capabilities it was drawn for are deleted. What is left to cross is smaller again than "the tray pushes and the hotkey edges": whether the ear is open turned out to be **state several surfaces read**, not a push at one of them, so it is `GET /api/listening` and off this seam entirely. What genuinely needs asking is the pushes with exactly one possible recipient — `flash`, `set_text`, raise-this-window, grab-this-screen. That doc's § *Open* carries the questions deliberately left unresolved. **Latency is not among them** — a loopback round trip measured 0.012 ms for a call and 0.29 ms for a 2 MB screen grab on an M4, three to four orders of magnitude under the mechanisms themselves, so the process boundary is never the thing to optimize on this seam. Keep `TCP_NODELAY` on; it is the one detail that turns those microseconds into tens of milliseconds.
