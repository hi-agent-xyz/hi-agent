...
**Surfaces are a separate axis from install shapes.** `crates/hi-app` is the roster + local proxy the face talks to; `app/apple/ios` and `app/android` are native phone/tablet clients. All three are API clients holding no engine state — they attach to a core, they are not one. The two mobile clients share no code and are not meant to: they speak the wire in [docs/api/client.md](docs/api/client.md) and are otherwise unrelated builds.
