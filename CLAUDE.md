...
**Status lives in [docs/status.md](docs/status.md), never in `docs/arch/`** — a design document that doubles as a status report goes stale in a way that makes readers distrust the design too. **That file holds one axis and nothing else: whether a mechanism has ever been observed running against a live instance.** It is the distinction this repo keeps paying for — **built is not watched** — and it is the only fact code, `docs/arch/` and `git log` cannot supply between them. Read it before starting architectural work, and update the row when something is watched, lands unwatched, or is named somewhere while absent from `src/`. **It explains nothing**: no mechanism, no rationale, no measurement, no history. A paragraph there describing how something works is a second source of truth beside the code, and it goes wrong silently — the entry is a name, a pointer, and either the date it was watched or the run that would settle it. A mechanism that is described and absent typechecks.
